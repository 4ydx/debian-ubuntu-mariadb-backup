...
do_backup () {
	...
	mariabackup --prepare --apply-log-only --user {$backup_owner} --target-dir "${full_backup_dir}"

	for increment in "${incremental_dirs[@]}"; do
		printf "Applying incremental backup %s to %s\n" "${increment}" "${full_backup_dir}"

		mariabackup --prepare --apply-log-only --user {$backup_owner} --incremental-dir="${increment}" --target-dir "${full_backup_dir}"
	done
...
Then, recreate the data directory and  copy the backup files:
				...
				sudo mariabackup --copy-back --user {$backup_owner} --target-dir ${PWD}/$(basename "${full_backup_dir}")
